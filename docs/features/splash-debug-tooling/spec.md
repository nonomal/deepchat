...
- Support `loading`, `system-unlock`, `unlock`, and `recovery` preview modes.
...
- Preserve real startup, database-unlock, and recovery ownership, validation, and sender restrictions.
...
- Preview requests use only `loading | system-unlock | unlock | recovery`; unknown modes fail route
  validation before reaching `SplashWindow`.
- Preview modes do not create an unlock or recovery request, access safeStorage, mutate
  database-security state, send password data, or start an empty database.
- Manual-unlock and recovery previews are visually representative but non-submittable: password
  fields and recovery actions are disabled, with a development-preview explanation.
...
| [Database recovery] [Close preview]                                  |
| Opening a preview disables the controls until the route resolves.    |
| Close preview is disabled while no preview is open.                  |
+---------------------------------------------------------------------+
```
...
| Database recovery        | Settings > Debug | Opens/reuses SplashWindow and selects a non-submittable `recovery` visual preview.           |
...
- The Vue splash renderer receives the mode through its preload subscription. It sets the visible
  branch directly and disables submissions in manual-unlock and recovery previews.
- The inline fallback renderer receives the same event and renders all four modes. It must retain
  existing real unlock and recovery IPC behavior.
- The splash BrowserWindow canvas and document root are transparent for loading and system credential
  ...
  password unlock and database recovery retain an opaque readable background behind their forms.
...
- Development Settings > Debug presents all four Splash preview controls and a close control.
...
- A selected preview is visible after SplashWindow loading and works on Vue and inline fallback
  renderers.
- Debug manual-unlock and recovery previews accept no password or recovery submission and do not
  mutate database state.
- Existing real loading, system credential unlock, manual password unlock, and database recovery
  flows keep their behavior.
