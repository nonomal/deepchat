...
const scan = async () => {
  ...
  scanning.value = true
  try {
    const result = await skillClient.scanGitSkillRepo(requestedRepoUrl)
    if (requestId !== scanRequestId || !isCurrentContext(contextVersion.value)) return
...
const install = async () => {
  ...
  await runInstall(async () => {
    ...
    const results = await skillClient.installFromGit(input)
    if (!isCurrentInstall(generation) || requestId !== installRequestId) return
    const surfaceCurrent =
      operationContextVersion === contextVersion.value && isCurrentContext(contextVersion.value)
