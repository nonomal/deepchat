...
def chrome_pid(binary, html_server, focus_monitor):
    ...
    # Use a throwaway profile dir so Chrome never shows restore/first-run dialogs
    import tempfile
    _tmp_profile = tempfile.mkdtemp(prefix="cua_chrome_test_")

    page_url = f"{html_server}/test_page.html"
    proc = subprocess.Popen(
        [
            CHROME_APP,
            f"--user-data-dir={_tmp_profile}",
...
class TestChromeButton:
    def test_click_increments_counter(self, driver, chrome_pid, ux_guard):
        wid = driver.find_window(chrome_pid, probe_ax=True)
...
class TestChromeTextInput:
    def test_type_text_appears(self, driver, chrome_pid, ux_guard):
        ...
        time.sleep(0.1)
        driver.type_text(chrome_pid, "hello chrome", delay_ms=30)
        time.sleep(0.6)
